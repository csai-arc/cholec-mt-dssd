...
#                   'sheep', 'sofa', 'train', 'tvmonitor')
    ...
    class_names = ('__background__', 'grasper', 'bipolar', 'hook',
                   'scissors', 'clipper', 'irrigator', 
                   'specimenbag')
    ...
    def __getitem__(self, index):
        image_id = self.ids[index]#.split()[0]
        image_id_split=self.ids[index].split()
        image_id=image_id_split[0]
        phase_labels=np.array(image_id_split[1], dtype=np.int64)
        tool_labels = np.array(image_id_split[2:], dtype=np.int64)
        #print(image_id_split,phase_labels,tool_labels)
        #sys.exit()
