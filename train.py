...
def train(cfg, args):
    ...
    #hl_graph = hl.build_graph(model, torch.zeros([10, 3, 320, 320]).to(device))

    # Use a different color theme
    #hl_graph.theme = hl.graph.THEMES["blue"].copy()  # Two options: basic and blue
    #hl_graph
    #save_image(hl_graph, 'hl_graph.png')
    #plt.plot(hl_graph)
    #plt.savefig('hl_graph.pdf')
    #sns_plot = sns.pairplot(hl_graph)
    #hl_graph.save("hl_graph_file")

    #torch.onnx.export(model.eval(), torch.zeros([1, 3, 320, 320]).to(device), 'model.onnx')
    #sys.exit()
